...
module.exports = async function () {
  ...
  describe('Success cases', function () {
    ...
    async function beforeTest(test, contracts, arg_item, deployer) {
      ...
      let finalNo = await contracts.LocalLucksVRF.viewRandomResult(arg_item.taskId);
      console.log("           prev finalNo:" + finalNo);

      if (finalNo < 1 || (!arg_item.random && finalNo != arg_item.max) ||
        (arg_item.random && finalNo > arg_item.max)) {
    ...
    tests.succes.forEach(function (succesTest) {
      ...
      it(succesTest.description, async function () {
        ...
        if (arg_item.random) {
          ...
          let finalNo = await contracts.LocalLucksVRF.connect(deployer).viewRandomResult(arg_item.taskId);

          expect(finalNo).to.greaterThan(0);
        }
        else {
          ...
          expect(finalNo).to.equal(arg_item.max);
        }
      });
    });
  });
