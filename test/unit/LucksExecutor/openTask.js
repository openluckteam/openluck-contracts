...
module.exports = function () {
  ...
  describe('Success cases', function () {
    ...
    it(`openTask tokenId-2 accept-USDC`, async function(){
      ...
      let taskId = 2;
      let task =  contracts.LucksExecutor.getTask(taskId);

      const viewRandomResult = await contracts.LucksVRF.viewRandomResult(taskId);
      if (task.status == 3) {
        let winNumber = await contracts.LucksExecutor.winTickets(taskId);
