...
contract LZEndpointMock is ILayerZeroEndpoint {
    ...
    function send(
        ...
        emit SendTo(destAddr, mockChainId);
        LZEndpointMock(lzEndpoint).receiveAndForward(destAddr, mockChainId, bytesSourceUserApplicationAddr, nonce, _payload);
    }

    event SendTo(address destAddr, uint16 _chainId);
    event ReceiveAndForward(address destAddr, uint16 _chainId);
    ...
    function receiveAndForward(
        ...
        emit ReceiveAndForward(_destAddr, _srcChainId);
